 ...
 */
static const t_u8 wmm_aci_to_qidx_map[] = {WMM_AC_BE, WMM_AC_BK, WMM_AC_VI, WMM_AC_VO};
 ...
 *  @return         ra_list
 */
static raListTbl *wlan_wmm_allocate_ralist_node(pmlan_adapter pmadapter, t_u8 *ra)
{
    raListTbl *ra_list = MNULL;

    ENTER();

    if (pmadapter->callbacks.moal_malloc(pmadapter->pmoal_handle, sizeof(raListTbl), MLAN_MEM_DEF, (t_u8 **)&ra_list))
    {
        PRINTM(MERROR, "Fail to allocate ra_list\n");
        goto done;
    }
    util_init_list((pmlan_linked_list)ra_list);
    util_init_list_head((t_void *)pmadapter->pmoal_handle, &ra_list->buf_head, MFALSE,
 ...
 * @return 	   N/A
 */
static void wlan_wmm_queue_priorities_tid(pmlan_private priv, t_u8 queue_priority[])
{
    int i;
    ...
    util_scalar_write(priv->adapter->pmoal_handle, &priv->wmm.highest_queued_prio, HIGH_PRIO_TID,
...
static mlan_wmm_ac_e wlan_wmm_eval_downgrade_ac(pmlan_private priv, mlan_wmm_ac_e eval_ac)
{
    ...
    for (down_ac = WMM_AC_BK; down_ac < eval_ac; down_ac++)
    {
        ...
        if ((pac_status->disabled == MFALSE) && (pac_status->flow_required == MFALSE))
 ...
 */
static mlan_wmm_ac_e INLINE wlan_wmm_convert_tos_to_ac(pmlan_adapter pmadapter, t_u32 tos)
{
    ENTER();
 ...
 */
static t_u8 INLINE wlan_wmm_downgrade_tid(pmlan_private priv, t_u32 tid)
{
    ...
    ac_down = priv->wmm.ac_down_graded_vals[wlan_wmm_convert_tos_to_ac(pmadapter, tid)];
    LEAVE();
    /*
 ...
 *  @return		N/A
 */
static INLINE void wlan_wmm_del_pkts_in_ralist_node(pmlan_private priv, raListTbl *ra_list)
{
    pmlan_buffer pmbuf;
    pmlan_adapter pmadapter = priv->adapter;

    ENTER();
    while ((pmbuf = (pmlan_buffer)util_peek_list(pmadapter->pmoal_handle, &ra_list->buf_head, MNULL, MNULL)))
    {
        util_unlink_list(pmadapter->pmoal_handle, &ra_list->buf_head, (pmlan_linked_list)pmbuf, MNULL, MNULL);
        wlan_write_data_complete(pmadapter, pmbuf, MLAN_STATUS_FAILURE);
    }
    util_free_list_head((t_void *)pmadapter->pmoal_handle, &ra_list->buf_head, pmadapter->callbacks.moal_free_lock);
 ...
 *  @return		N/A
 */
static INLINE void wlan_wmm_del_pkts_in_ralist(pmlan_private priv, mlan_list_head *ra_list_head)
{
    raListTbl *ra_list;

    ENTER();

    ra_list = (raListTbl *)util_peek_list(priv->adapter->pmoal_handle, ra_list_head, MNULL, MNULL);
...
static void wlan_wmm_cleanup_queues(pmlan_private priv)
{
    ...
    util_scalar_write(priv->adapter->pmoal_handle, &priv->wmm.tx_pkts_queued, 0, MNULL, MNULL);
    util_scalar_write(priv->adapter->pmoal_handle, &priv->wmm.highest_queued_prio, HIGH_PRIO_TID, MNULL, MNULL);
 ...
 *   @return         ra_list
 */
static raListTbl *wlan_wmm_get_queue_raptr(pmlan_private priv, t_u8 tid, t_u8 *ra_addr)
{
    raListTbl *ra_list;
 ...
 *  @return		N/A
 */
static void wlan_send_wmmac_host_event(pmlan_private priv, char *typeStr, t_u8 *srcAddr, t_u8 tid, t_u8 up, t_u8 status)
{
    t_u8 event_buf[100];
    mlan_event *pevent;
    t_u8 *pOutBuf;
    ...
    (void)__memcpy(priv->adapter, pOutBuf, (t_u8 *)"TSPEC:", 6);
    pOutBuf += 6;

    (void)__memcpy(priv->adapter, pOutBuf, (t_u8 *)typeStr, wlan_strlen(typeStr));
    ...
    *pOutBuf++ = status + '0';
    ...
    (void)__memcpy(priv->adapter, pOutBuf, (t_u8 *)"]:TID", 5);
    pOutBuf += 5;
    *pOutBuf++ = tid + '0';

    (void)__memcpy(priv->adapter, pOutBuf, (t_u8 *)":UP", 3);
    pOutBuf += 3;
    *pOutBuf++ = up + '0';
 ...
 *  @return             raListTbl
 */
static raListTbl *wlan_wmm_get_highest_priolist_ptr(pmlan_adapter pmadapter, pmlan_private *priv, int *tid)
{
    ...
    {
        if (!(util_peek_list(pmadapter->pmoal_handle, &pmadapter->bssprio_tbl[j].bssprio_head,
                             pmadapter->callbacks.moal_spin_lock, pmadapter->callbacks.moal_spin_unlock)))
            continue;

        if (pmadapter->bssprio_tbl[j].bssprio_cur == (mlan_bssprio_node *)&pmadapter->bssprio_tbl[j].bssprio_head)
        {
            pmadapter->bssprio_tbl[j].bssprio_cur = pmadapter->bssprio_tbl[j].bssprio_cur->pnext;
        }
        ...
        do
        {
            ...
            if ((priv_tmp->port_ctrl_mode == MTRUE) && (priv_tmp->port_open == MFALSE))
            {
                PRINTM(MINFO,
            ...
            pmadapter->callbacks.moal_spin_lock(pmadapter->pmoal_handle, priv_tmp->wmm.ra_list_spinlock);

            for (i = util_scalar_read(pmadapter->pmoal_handle, &priv_tmp->wmm.highest_queued_prio, MNULL, MNULL);
                 i >= LOW_PRIO_TID; --i)
            {
                tid_ptr = &(priv_tmp)->wmm.tid_tbl_ptr[tos_to_tid[i]];
                if (!util_peek_list(pmadapter->pmoal_handle, &tid_ptr->ra_list, MNULL, MNULL))
                    continue;
                ...
                do
                {
                    if (!ptr->tx_pause && util_peek_list(pmadapter->pmoal_handle, &ptr->buf_head, MNULL, MNULL))
                    {
                        ...
                        if (priv_tmp->wmm.pkts_queued[next_tid])
                            util_scalar_write(pmadapter->pmoal_handle, &priv_tmp->wmm.highest_queued_prio, next_prio,
                                              MNULL, MNULL);
                        else
                            /* if highest_queued_prio > i, set it to i */
                            util_scalar_conditional_write(pmadapter->pmoal_handle, &priv_tmp->wmm.highest_queued_prio,
                                                          MLAN_SCALAR_COND_GREATER_THAN, i, i, MNULL, MNULL);
                        *priv = priv_tmp;
                        *tid  = tos_to_tid[i];
            ...
            util_scalar_write(pmadapter->pmoal_handle, &priv_tmp->wmm.highest_queued_prio, NO_PKT_PRIO_TID, MNULL,
                              MNULL);

            pmadapter->callbacks.moal_spin_unlock(pmadapter->pmoal_handle, priv_tmp->wmm.ra_list_spinlock);

        next_intf:
            if ((bssprio_node = bssprio_node->pnext) == (mlan_bssprio_node *)&pmadapter->bssprio_tbl[j].bssprio_head)
 ...
 *  @return             Packet count
 */
static int wlan_num_pkts_in_txq(mlan_private *priv, raListTbl *ptr, int maxBufSize)
{
    ...
    for (pmbuf = (pmlan_buffer)ptr->buf_head.pnext; pmbuf != (pmlan_buffer)(&ptr->buf_head); pmbuf = pmbuf->pnext)
    {
 ...
 *  @return             N/A
 */
static void INLINE wlan_send_single_packet(pmlan_private priv, raListTbl *ptr, int ptrindex)
{
    pmlan_buffer pmbuf;
    ...
    if ((pmbuf = (pmlan_buffer)util_dequeue_list(pmadapter->pmoal_handle, &ptr->buf_head, MNULL, MNULL)))
    {
        PRINTM(MINFO, "Dequeuing the packet %p %p\n", ptr, pmbuf);
        priv->wmm.pkts_queued[ptrindex]--;
        util_scalar_decrement(pmadapter->pmoal_handle, &priv->wmm.tx_pkts_queued, MNULL, MNULL);
        ptr->total_pkts--;
        pmbuf_next = (pmlan_buffer)util_peek_list(pmadapter->pmoal_handle, &ptr->buf_head, MNULL, MNULL);
        pmadapter->callbacks.moal_spin_unlock(pmadapter->pmoal_handle, priv->wmm.ra_list_spinlock);

        tx_param.next_pkt_len = ((pmbuf_next) ? pmbuf_next->data_len + sizeof(TxPD) : 0);
        ...
        if (status == MLAN_STATUS_RESOURCE)
        {
            ...
            priv->wmm.pkts_queued[ptrindex]++;
            util_scalar_increment(pmadapter->pmoal_handle, &priv->wmm.tx_pkts_queued, MNULL, MNULL);
            util_enqueue_list_head(pmadapter->pmoal_handle, &ptr->buf_head, (pmlan_linked_list)pmbuf, MNULL, MNULL);
 ...
 */
static int INLINE wlan_is_ptr_processed(mlan_private *priv, raListTbl *ptr)
{
    pmlan_buffer pmbuf;

    if ((pmbuf = (pmlan_buffer)util_peek_list(priv->adapter->pmoal_handle, &ptr->buf_head, MNULL, MNULL)) &&
        (pmbuf->flags & MLAN_BUF_FLAG_REQUEUED_PKT))
        return MTRUE;
 ...
 *  @return             N/A
 */
static void INLINE wlan_send_processed_packet(pmlan_private priv, raListTbl *ptr, int ptrindex)
{
    pmlan_buffer pmbuf_next;
    mlan_tx_param tx_param;
    pmlan_buffer pmbuf;
...
static int wlan_dequeue_tx_packet(pmlan_adapter pmadapter)
{
    ...
    if (!ptr->is_11n_enabled || wlan_is_bastream_setup(priv, ptr, tid)
        ...
        || ((priv->sec_info.ewpa_enabled == MFALSE) &&
            ((priv->sec_info.wpa_enabled || priv->sec_info.wpa2_enabled) && priv->wpa_is_gtk_set == MFALSE)) ||
        priv->wps.session_enable
#endif /* STA_SUPPORT */
    )
    {
        if (ptr->is_11n_enabled && wlan_is_bastream_setup(priv, ptr, tid) &&
            wlan_is_amsdu_in_ampdu_allowed(priv, ptr, tid) && wlan_is_amsdu_allowed(priv, ptr, tid) &&
            (wlan_num_pkts_in_txq(priv, ptr, pmadapter->tx_buf_size) >= MIN_NUM_AMSDU))
        {
    ...
    {
        if (wlan_is_ampdu_allowed(priv, ptr, tid) && (ptr->packet_count > ptr->ba_packet_threshold))
        {
...
t_void wlan_updata_ralist_tx_pause(pmlan_private priv, t_u8 *mac, t_u8 tx_pause)
{
    ...
    if (pkt_cnt)
    {
        tx_pkts_queued = util_scalar_read(pmadapter->pmoal_handle, &priv->wmm.tx_pkts_queued, MNULL, MNULL);
        if (tx_pause)
            tx_pkts_queued -= pkt_cnt;
        else
            tx_pkts_queued += pkt_cnt;
        util_scalar_write(priv->adapter->pmoal_handle, &priv->wmm.tx_pkts_queued, tx_pkts_queued, MNULL, MNULL);
        util_scalar_write(priv->adapter->pmoal_handle, &priv->wmm.highest_queued_prio, HIGH_PRIO_TID, MNULL, MNULL);
    }
    pmadapter->callbacks.moal_spin_unlock(pmadapter->pmoal_handle, priv->wmm.ra_list_spinlock);
    LEAVE();
...
#define BA_SETUP_PACKET_OFFSET        16
    ...
    pmadapter->callbacks.moal_get_system_time(pmadapter->pmoal_handle, &sec, &usec);
    sec  = (sec & 0xFFFF) + (sec >> 16);
    usec = (usec & 0xFFFF) + (usec >> 16);

    ba_threshold = (((sec << 16) + usec) % BA_SETUP_MAX_PACKET_THRESHOLD) + BA_SETUP_PACKET_OFFSET;
 ...
 *  @return         N/A
 */
void wlan_wmm_setup_queue_priorities(pmlan_private priv, IEEEtypes_WmmParameter_t *pwmm_ie)
{
    ...
    HEXDUMP("WMM: setup_queue_priorities: param IE", (t_u8 *)pwmm_ie, sizeof(IEEEtypes_WmmParameter_t));

    PRINTM(MINFO,
           "WMM Parameter IE: version=%d, "
           "qos_info Parameter Set Count=%d, Reserved=%#x\n",
           pwmm_ie->vend_hdr.version, pwmm_ie->qos_info.para_set_count, pwmm_ie->reserved);

    for (num_ac = 0; num_ac < NELEMENTS(pwmm_ie->ac_params); num_ac++)
    {
        cw_min       = (1 << pwmm_ie->ac_params[num_ac].ecw.ecw_min) - 1;
        avg_back_off = (cw_min >> 1) + pwmm_ie->ac_params[num_ac].aci_aifsn.aifsn;
    ...
    HEXDUMP("WMM: queue_priority", priv->wmm.queue_priority, sizeof(priv->wmm.queue_priority));
    ...
    for (i = 0; i < num_ac; i++)
    {
        for (j = 1; j < num_ac - i; j++)
        {
            if (tmp[j - 1] > tmp[j])
            {
                ...
                SWAP_U8(priv->wmm.queue_priority[j - 1], priv->wmm.queue_priority[j]);
            }
            else if (tmp[j - 1] == tmp[j])
            {
                if (priv->wmm.queue_priority[j - 1] < priv->wmm.queue_priority[j])
                {
                    SWAP_U8(priv->wmm.queue_priority[j - 1], priv->wmm.queue_priority[j]);
                }
            }
    ...
    DBG_HEXDUMP(MCMD_D, "WMM: queue_priority, sort", priv->wmm.queue_priority, sizeof(priv->wmm.queue_priority));
    LEAVE();
}
...
void wlan_wmm_setup_ac_downgrade(pmlan_private priv)
{
    ...
    else
    {
        for (ac_val = WMM_AC_BK; ac_val <= WMM_AC_VO; ac_val++)
        {
            priv->wmm.ac_down_graded_vals[ac_val] = wlan_wmm_eval_downgrade_ac(priv, (mlan_wmm_ac_e)ac_val);
...
void wlan_ralist_add(mlan_private *priv, t_u8 *ra)
{
    ...
    for (i = 0; i < MAX_NUM_TID; ++i)
    {
        ...
        if (ra_list->is_11n_enabled)
        {
            ...
            ra_list->ba_packet_threshold = wlan_get_random_ba_threshold(pmadapter);
        }
...
int wlan_wmm_lists_empty(pmlan_adapter pmadapter)
{
    ...
    for (j = 0; j < pmadapter->priv_num; ++j)
    {
        if ((priv = pmadapter->priv[j]))
        {
            ...
            {
                PRINTM(MINFO, "wmm_lists_empty: PORT_CLOSED Ignore pkts from BSS%d\n", j);
                continue;
            }

            if (util_scalar_read(pmadapter->pmoal_handle, &priv->wmm.tx_pkts_queued,
                                 pmadapter->callbacks.moal_spin_lock, pmadapter->callbacks.moal_spin_unlock))
            {
                LEAVE();
                return MFALSE;
...
int wlan_is_ralist_valid(mlan_private *priv, raListTbl *ra_list, int ptrindex)
{
    ...
    rlist = (raListTbl *)util_peek_list(priv->adapter->pmoal_handle, &priv->wmm.tid_tbl_ptr[ptrindex].ra_list, MNULL,
                                        MNULL);

    while (rlist && (rlist != (raListTbl *)&priv->wmm.tid_tbl_ptr[ptrindex].ra_list))
    {
        if (rlist == ra_list)
        {
            LEAVE();
    ...
    for (tid = 0; tid < MAX_NUM_TID; ++tid)
    {
        ...
        if (ra_list)
        {
            ...
            PRINTM(MINFO,
                   ...
                   ra_list, ra_list->is_11n_enabled, ra_list->ra[0], ra_list->ra[1], ra_list->ra[2], ra_list->ra[3],
                   ra_list->ra[4], ra_list->ra[5], new_ra[0], new_ra[1], new_ra[2], new_ra[3], new_ra[4], new_ra[5]);
...
t_void wlan_wmm_add_buf_txqueue(pmlan_adapter pmadapter, pmlan_buffer pmbuf)
{
    ...
    if (!queuing_ra_based(priv))
    {
        ra_list = (raListTbl *)util_peek_list(pmadapter->pmoal_handle, &priv->wmm.tid_tbl_ptr[tid_down].ra_list, MNULL,
                                              MNULL);
    }
    else
    {
        /* Note: We do not have data @ some offset of pbuf. pbuf only has RxPD */
        (void)__memcpy(pmadapter, ra, pmbuf->pdesc, MLAN_MAC_ADDR_LENGTH);
        /** put multicast/broadcast packet in the same ralist */
        if (ra[0] & 0x01)
            (void)__memset(pmadapter, ra, 0xff, sizeof(ra));
    ...
    util_enqueue_list_tail(pmadapter->pmoal_handle, &ra_list->buf_head, (pmlan_linked_list)pmbuf, MNULL, MNULL);
    ...
    if (!ra_list->tx_pause)
    {
        ...
        util_scalar_conditional_write(pmadapter->pmoal_handle, &priv->wmm.highest_queued_prio,
                                      MLAN_SCALAR_COND_LESS_THAN, tos_to_tid_inv[tid_down], tos_to_tid_inv[tid_down],
                                      MNULL, MNULL);
    }
 ...
 *  @return MLAN_STATUS_SUCCESS
 */
mlan_status wlan_ret_wmm_get_status(pmlan_private priv, t_u8 *ptlv, int resp_len)
{
    t_u8 *pcurrent = ptlv;
    t_u32 tlv_len;
    t_u8 sendWmmEvent;
    ...
    while (resp_len >= sizeof(pTlvHdr->header))
    {
        ...
        switch (wlan_le16_to_cpu(pTlvHdr->header.type))
        {
            ...
            case WMM_IE:
                ...
                (void)__memcpy(priv->adapter, (t_u8 *)&priv->curr_bss_params.bss_descriptor.wmm_ie, pWmmParamIe,
                               MIN(sizeof(IEEEtypes_WmmParameter_t), (pWmmParamIe->vend_hdr.len + 2)));
                sendWmmEvent = MTRUE;
                break;
            ...
            case TLV_TYPE_IEEE_ACTION_FRAME:
                ...
                if (pTlvAction->actionFrame.wmmAc.tspecAct.category == IEEE_MGMT_ACTION_CATEGORY_WMM_TSPEC)
                {
                    switch (pTlvAction->actionFrame.wmmAc.tspecAct.action)
                    {
                        case TSPEC_ACTION_CODE_ADDTS_RSP:
                            ...
                            wlan_send_wmmac_host_event(
                                priv, "ADDTS_RSP", pTlvAction->srcAddr, pAddTsRsp->tspecIE.TspecBody.TSInfo.TID,
                                pAddTsRsp->tspecIE.TspecBody.TSInfo.UserPri, pAddTsRsp->statusCode);
                            break;

                        case TSPEC_ACTION_CODE_DELTS:
                            pDelTs = &pTlvAction->actionFrame.wmmAc.delTs;
                            wlan_send_wmmac_host_event(priv, "DELTS_RX", pTlvAction->srcAddr,
                                                       pDelTs->tspecIE.TspecBody.TSInfo.TID,
                                                       pDelTs->tspecIE.TspecBody.TSInfo.UserPri, pDelTs->reasonCode);
                            break;
...
t_u8 wlan_wmm_compute_driver_packet_delay(pmlan_private priv, const pmlan_buffer pmbuf)
{
    ...
    priv->adapter->callbacks.moal_get_system_time(priv->adapter->pmoal_handle, &out_ts_sec, &out_ts_usec);
...
void wlan_wmm_process_tx(pmlan_adapter pmadapter)
{
    ...
    do
    {
        if (wlan_dequeue_tx_packet(pmadapter))
            break;
        /* Check if busy */
    } while (!pmadapter->data_sent && !pmadapter->tx_lock_flag && !wlan_wmm_lists_empty(pmadapter));
...
t_u8 wlan_wmm_select_queue(mlan_private *pmpriv, t_u8 tid)
{
    ...
    t_u8 i;
    mlan_wmm_ac_e ac_down = pmpriv->wmm.ac_down_graded_vals[wlan_wmm_convert_tos_to_ac(pmadapter, tid)];
 ...
 *  @return		N/A
 */
static INLINE t_u8 wlan_del_tx_pkts_in_ralist(pmlan_private priv, mlan_list_head *ra_list_head, int tid)
{
    raListTbl *ra_list      = MNULL;
    pmlan_adapter pmadapter = priv->adapter;
    ...
    while (ra_list && ra_list != (raListTbl *)ra_list_head)
    {
        if (ra_list->total_pkts && (ra_list->tx_pause || (ra_list->total_pkts > RX_LOW_THRESHOLD)))
        {
            if ((pmbuf = (pmlan_buffer)util_dequeue_list(pmadapter->pmoal_handle, &ra_list->buf_head, MNULL, MNULL)))
            {
                PRINTM(MDATA, "Drop pkts: tid=%d tx_pause=%d pkts=%d brd_pkts=%d %02x:%02x:%02x:%02x:%02x:%02x\n", tid,
                       ra_list->tx_pause, ra_list->total_pkts, pmadapter->pending_bridge_pkts, ra_list->ra[0],
                       ra_list->ra[1], ra_list->ra[2], ra_list->ra[3], ra_list->ra[4], ra_list->ra[5]);
                wlan_write_data_complete(pmadapter, pmbuf, MLAN_STATUS_FAILURE);
...
t_void wlan_drop_tx_pkts(pmlan_private priv)
{
    ...
    for (j = 0; j < MAX_NUM_TID; j++, i++)
    {
        if (i == MAX_NUM_TID)
            i = 0;
        if (wlan_del_tx_pkts_in_ralist(priv, &priv->wmm.tid_tbl_ptr[i].ra_list, i))
        {
            i++;
            break;
        }
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_cmd_wmm_addts_req(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    (void)__memcpy(pmpriv->adapter, pcmd_addts->tspec_data, paddts->ie_data, MIN(WMM_TSPEC_SIZE, paddts->ie_data_len));
                                   ...
                                   OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        if (presp_addts->command_result == MLAN_CMD_RESULT_SUCCESS)
        {
            ...
            paddts->ie_data_len =
                (t_u8)(resp->size - sizeof(presp_addts->command_result) - sizeof(presp_addts->timeout_ms) -
                       sizeof(presp_addts->dialog_token) - sizeof(presp_addts->ieee_status_code) - S_DS_GEN);

            /* Copy the TSPEC data include any extra IEs after the TSPEC */
            (void)__memcpy(pmpriv->adapter, paddts->ie_data, presp_addts->tspec_data, paddts->ie_data_len);
        }
        else
        {
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_cmd_wmm_delts_req(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
                                   ...
                                   OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        if (presp_delts->command_result == 0)
        {
            ...
            wlan_send_wmmac_host_event(pmpriv, "DELTS_TX", MNULL, pTspecIE->TspecBody.TSInfo.TID,
                                       pTspecIE->TspecBody.TSInfo.UserPri, presp_delts->ieee_reason_code);
        }
    }
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_cmd_wmm_queue_config(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    pcmd_qcfg->msdu_lifetime_expiry = wlan_cpu_to_le16(pqcfg->msdu_lifetime_expiry);
                                      ...
                                      OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        pwmm->param.q_cfg.msdu_lifetime_expiry = wlan_le16_to_cpu(presp_qcfg->msdu_lifetime_expiry);
    }
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_cmd_wmm_queue_stats(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    cmd->size    = wlan_cpu_to_le16(sizeof(HostCmd_DS_WMM_QUEUE_STATS) + S_DS_GEN);
    cmd->result  = 0;
    ...
    {
        pcmd_qstats->delay_histogram[id] = wlan_cpu_to_le16(pqstats->delay_histogram[id]);
    }
                                     ...
                                     OUT mlan_ioctl_req *pioctl_buf)
{
    ...
    if (pioctl_buf)
    {
        ...
        pqstats->avg_queue_delay = wlan_le32_to_cpu(presp_qstats->avg_queue_delay);
        ...
        for (id = 0; id < MLAN_WMM_STATS_PKTS_HIST_BINS; id++)
        {
            pqstats->delay_histogram[id] = wlan_le16_to_cpu(presp_qstats->delay_histogram[id]);
        }
    }
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_cmd_wmm_ts_status(IN pmlan_private pmpriv, OUT HostCmd_DS_COMMAND *cmd, IN t_void *pdata_buf)
{
    ...
    (void)__memcpy(pmpriv->adapter, (t_void *)pcmd_ts_status, (t_void *)pts_status, sizeof(HostCmd_DS_WMM_TS_STATUS));
 ...
 *  @return             MLAN_STATUS_SUCCESS
 */
mlan_status wlan_ret_wmm_ts_status(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *resp, OUT mlan_ioctl_req *pioctl_buf)
{
    mlan_ds_wmm_cfg *pwmm                     = MNULL;
    ...
    {
        pwmm                         = (mlan_ds_wmm_cfg *)pioctl_buf->pbuf;
        presp_ts_status->medium_time = wlan_le16_to_cpu(presp_ts_status->medium_time);
        (void)__memcpy(pmpriv->adapter, (t_void *)&pwmm->param.ts_status, (t_void *)presp_ts_status,
